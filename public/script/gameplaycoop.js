...
// ==========================================
// 4. CLASSES DES OBSTACLES & ITEMS (Mode Plateformes Plates)
// ==========================================
class Rouleau {
    constructor(x, y) {
        this.x = x; this.y = y; this.w = 45; this.h = 45; 
        // Part soit à gauche (-3), soit à droite (3) aléatoirement
        this.vx = Math.random() > 0.5 ? 3 : -3; 
        this.vy = 0; this.angle = 0;
    }
    bouger() {
        this.x += this.vx; this.y += this.vy; 
        // Tourne dans le sens du mouvement
        this.angle += (this.vx > 0 ? 0.05 : -0.05); 

        // Rebondit sur les murs latéraux pour ne pas sortir de l'écran avant le bas
        if (this.y + this.h < 790) { 
            if (this.x <= 0) { this.x = 0; this.vx = Math.abs(this.vx); }
            if (this.x + this.w >= canvasW) { this.x = canvasW - this.w; this.vx = -Math.abs(this.vx); }
        }

        let estSurPlateforme = false;
        for (let plat of listePlateformes) {
            // Plus d'inclinaison, y est constant
            if (this.x + this.w / 2 > plat.x && this.x + this.w / 2 < plat.x + plat.w &&
                this.y + this.h >= plat.y - 10 && this.y + this.h - this.vy <= plat.y + 20) {
                estSurPlateforme = true; 
                this.y = plat.y - this.h; 
                this.vy = 0;
                // Contrairement aux pentes, il garde juste sa vitesse constante !
                break;
            }
        }
        
        if (!estSurPlateforme) {
            this.vy += 0.5; // Tombe si y'a un trou
            if (this.y + this.h >= 800) { 
                this.y = 800 - this.h; this.vy = 0; 
            } 
        }
    }
}
...
class TomateTombante {
    constructor() { this.x = Math.random() * (canvasW - 100) + 50; this.y = -50; this.w = 25; this.h = 25; this.vy = Math.random() * 2 + 3; }
    bouger() { this.y += this.vy; }
}

class CouteauTombante {
    constructor() { this.x = Math.random() * (canvasW - 100) + 50; this.y = -50; this.w = 20; this.h = 60; this.vy = 5; }
    bouger() { this.y += this.vy; }
}
...
class CollectablePowerup {
    constructor(x, y, type) { this.x = x; this.y = y; this.w = 40; this.h = 40; this.type = type; }
}

// ==========================================
// 5. DESIGN DU NIVEAU (PLATEFORMES PLATES)
// ==========================================
const clavier = new Clavier();
const monRat = new Rat(100, 700, "joueur1", "gris"); 
const ratCobaye = new Rat(800, 700, "joueur2", "marron"); 
let listeJoueurs = [monRat, ratCobaye]; 
let etatPartie = "EN_COURS"; 
const canvasW = 900; const canvasH = 900;

// NOUVEAU LEVEL DESIGN : Plateformes 100% plates avec des trous
const listePlateformes = [
    { x: 0, y: 800, w: 900, h: 18, inclinaison: 0 },   // Sol Global
    { x: 100, y: 650, w: 700, h: 18, inclinaison: 0 }, // Étage 1 (Trous sur les bords)
    { x: 0, y: 500, w: 350, h: 18, inclinaison: 0 },   // Étage 2 Gauche
    { x: 550, y: 500, w: 350, h: 18, inclinaison: 0 }, // Étage 2 Droite (Trou au milieu)
    { x: 100, y: 350, w: 700, h: 18, inclinaison: 0 }, // Étage 3 (Trous sur les bords)
    { x: 0, y: 200, w: 350, h: 18, inclinaison: 0 },   // Étage 4 Gauche
    { x: 550, y: 200, w: 350, h: 18, inclinaison: 0 }, // Étage 4 Droite (Trou au milieu)
    { x: 300, y: 80, w: 300, h: 18, inclinaison: 0 }   // Étage du Boss (Centre)
];

function getPlatformY(plat, targetX) {
    return plat.y; // Plus de pente, c'est tout plat !
}

// Échelles adaptées aux nouvelles plateformes
const listeEchelles = [
    { x: 200, w: 30, yTop: 650, yBot: 800 },
    { x: 700, w: 30, yTop: 650, yBot: 800 },
    { x: 300, w: 30, yTop: 500, yBot: 650 },
    { x: 570, w: 30, yTop: 500, yBot: 650 },
    { x: 200, w: 30, yTop: 350, yBot: 500 },
    { x: 700, w: 30, yTop: 350, yBot: 500 },
    { x: 300, w: 30, yTop: 200, yBot: 350 },
    { x: 570, w: 30, yTop: 200, yBot: 350 },
    { x: 435, w: 30, yTop: 80,  yBot: 200 }
];

let listeRouleaux = []; let listeTomates = []; let listeCouteaux = []; let listeCoeurs = []; let listePowerupsCollectables = [];

let listeLeviers = [
    new LevierItem(50, 800 - 50),
    new LevierItem(820, 800 - 50),
    new LevierItem(435, 350 - 50) 
];
let fromageVictorieux = new FromageVictorieux(425, 40); 
...
// 6. SYSTÈME DE SPAWN 
// ==========================================
const BOSS_SPAWN = { x: 425, y: 30 }; // Spawn au centre tout en haut !
...
setInterval(() => {
    if (etatPartie === "EN_COURS" && listeCoeurs.length < 3) {
        ...
        const rx = plat.x + Math.random() * (plat.w - 50); const ry = plat.y - 30;
...
function boucleDeJeu() {
            ...
            if (!joueur.enEscalade && joueur.vy >= 0 && joueur.ignorePlateformesTimer <= 0) {
                ...
                for (let plat of listePlateformes) {
                    ...
                    if (rxCentre >= plat.x && rxCentre <= plat.x + plat.w) {
                        if (joueur.y + boiteJoueur.h >= plat.y - 10 && joueur.y + boiteJoueur.h <= plat.y + 15) {
                            joueur.y = plat.y - boiteJoueur.h; joueur.vy = 0; joueur.peutSauter = true; surPlateforme = true; break;
                        }
                    }
                }
    ...
    if (etatPartie === "EN_COURS") {
        for (let i = listeRouleaux.length - 1; i >= 0; i--) { 
            let obs = listeRouleaux[i]; obs.bouger(); 
            // Despawn par les côtés une fois en bas
    ...
    for (let plat of listePlateformes) { ctx.fillStyle = "#CD853F"; ctx.strokeStyle = "#8B4513"; ctx.fillRect(plat.x, plat.y, plat.w, plat.h); ctx.strokeRect(plat.x, plat.y, plat.w, plat.h); }
    ...
    for (let p of listePowerupsCollectables) { dessinerSprite(imgPanier, p.x, p.y, p.w, p.h, "cyan"); }
